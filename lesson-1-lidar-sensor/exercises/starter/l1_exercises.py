...
def print_pitch_resolution(frame, lidar_name):
    ...
    vertical_fov = max_pitch - min_pitch #this is in radians

    # compute pitch resolution and convert it to angular minutes
    pitch_resolution_rad = vertical_fov / ri.shape[0]
    pitch_resolution_deg = pitch_resolution_rad * 180 / np.pi
    print('pitch angle resolution = {0:.2f} degrees'.format(pitch_resolution_deg))
...
def print_no_of_vehicles(frame):
    ...
    for item in frame.laser_labels:
        if (item.type == item.TYPE_VEHICLE):
            num_vehicles += 1
